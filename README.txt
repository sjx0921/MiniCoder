...
特色：持久会话与 /reset；目录列举、文本搜索、读写/精确替换文件、命令执行、Git 状态与差异审查；文件工具限制在工作区；写文件和命令默认要求确认，--auto-approve 仅自动批准低、中风险操作；递归删除、破坏性 Git 操作等高风险命令始终确认；支持任务计划、最大轮数、命令超时、输出截断、结构化上下文摘要和结束时 Git 改动报告。离线测试不需要 API Key。
